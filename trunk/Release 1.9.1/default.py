...
class MainWindow(xbmcgui.Window):
        def __init__(self):
            ...
            self.logo = xbmcgui.ControlImage(610,20,80,80, imageDir + "logo.png")
            self.addControl(self.logo)
            
            #user logo below the buttons
            self.user_logo = xbmcgui.ControlImage(35,315,160,140, imageDir + "logo.png")
            self.addControl(self.user_logo)
            self.user_logo.setVisible(0)
            
            #user thumb below the buttons
            self.user_thumb = xbmcgui.ControlImage(35,315,160,140, imageDir + "logo.png")
            self.addControl(self.user_thumb)
            self.user_thumb.setVisible(0)
            
            #URL label at top-center position
            self.urllbl = xbmcgui.ControlLabel(210,60,350,40, "", "font14")
            self.addControl(self.urllbl)

            #Version label at top-left position
            self.version = xbmcgui.ControlLabel(55,80,155,40, 'version: '+ Version+ '.' + SubVersion, "font14")
            self.addControl(self.version)      

            #loading text at down-left side
            self.infotekst = xbmcgui.ControlLabel(80,500,250,30, "Loading...")
            self.addControl(self.infotekst)
            self.infotekst.setVisible(0)

            #please wait text displayed on top of the main list
            self.loading = xbmcgui.ControlLabel(220, 140,250,30, "Please wait...")
            self.addControl(self.loading)
            self.loading.setVisible(0)

            #main list displaying playlist entries
            self.list = xbmcgui.ControlList(210, 140, 470 , 400,'font14','0xFFDDDDDD', buttonFocusTexture=RootDir+'images\\list-focus.png', imageWidth=36, imageHeight=36, itemHeight=42)
            self.addControl(self.list)
            self.list.setPageControlVisible(False)

            #main list position
            self.listpos = xbmcgui.ControlLabel(620,520,250,30, "")
            self.addControl(self.listpos)
            
            #buttons creation
            self.button_home = xbmcgui.ControlButton(35,115,160,32,'Home', RootDir + "images\\button-focus.png", RootDir + "images\\button-nofocus.png")
            self.addControl(self.button_home)
            self.button_favorites = xbmcgui.ControlButton(35,155,160,32,'Favorites', RootDir + "images\\button-focus.png", RootDir + "images\\button-nofocus.png")            
            self.addControl(self.button_favorites)            
            self.button_downloads = xbmcgui.ControlButton(35,195,160,32,'Downloads', RootDir + "images\\button-focus.png", RootDir + "images\\button-nofocus.png")            
            self.addControl(self.button_downloads)            
            self.button_url = xbmcgui.ControlButton(35,235,160,32,'Browse', RootDir + "images\\button-focus.png", RootDir + "images\\button-nofocus.png")
            self.addControl(self.button_url)
            self.button_about = xbmcgui.ControlButton(35,275,160,32,'About', RootDir + "images\\button-focus.png", RootDir + "images\\button-nofocus.png")
            self.addControl(self.button_about)
            
            #button key behaviour
            self.button_home.controlDown(self.button_favorites)
            self.button_favorites.controlDown(self.button_downloads)
            self.button_downloads.controlDown(self.button_url)
            self.button_url.controlDown(self.button_about)
            self.button_about.controlUp(self.button_url)
            self.button_url.controlUp(self.button_downloads)
            self.button_downloads.controlUp(self.button_favorites)
            self.button_favorites.controlUp(self.button_home)
            ...
            self.list.controlLeft(self.button_home)
